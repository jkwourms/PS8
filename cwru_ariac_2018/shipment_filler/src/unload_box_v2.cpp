...
int main(int argc, char** argv) {
	...
	//use robotBehaviorInterface to remove current part from box
    ...
    cout<<"enter 1 to attempt to remove the other parts: "; //poor-man's breakpoint
    cin>>ans;

    //cycle through all parts listed as orphans & set as current part to remove
    for (int i=0;i<nparts;i++) {
       ROS_INFO_STREAM("orphaned  parts: "<<orphan_models_wrt_world[i]<<endl);
